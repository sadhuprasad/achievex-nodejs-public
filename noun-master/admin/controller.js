...
exports.customerList = async (req, res) => {
    let sql = "SELECT * FROM `users` WHERE role='customer' AND status='active'";
...
// customerDetails
exports.customerCars = async (req, res) => {
    let customerId = req.params.id;
    let sql = "SELECT `users`.`name`, `users`.`email`, `users`.`phone`, `customer_cars`.`carKey`, `customer_cars`.`status` AS customerCarStatus, `car_details`.`carBrand`, `car_details`.`carModel`, `car_details`.`carSubModel`, CONCAT('https://noun.achievextesting.one/storage/docs/', `car_details`.`carImage`) AS carImage, `charging_ports`.`portName`, `charging_ports`.`portType` FROM `users`, `customer_cars`, `car_details`, `charging_ports` WHERE `users`.`id` = `customer_cars`.`customerKey` AND `customer_cars`.`carKey` = `car_details`.`carKey` AND `car_details`.`chargingPortKey`=`charging_ports`.`portKey` AND `users`.`id` = ?";
    DB.query(sql, [customerId], async (err, result) => {
        if (err) {
            return res
                .status(200)
                .json({
...
exports.customerBookingDetails = async (req, res) => {
    let customerId = req.params.id;
    let sql = "SELECT `users`.`name`, `users`.`email`, `users`.`phone`, `booking_details`.`bookingType`, `booking_details`.`bookingDate`, `booking_details`.`price`, `booking_details`.`grandTotalPrice`, `service_stations`.`stationName`, `service_stations`.`contactNumber`, `service_stations`.`stationAddress`, `service_stations`.`status` AS stationStatus, `station_ports`.`fldStationPortName`, `station_ports`.`fldStationPortPrice`, `station_ports`.`powerConsumption`, `station_ports`.`status` AS portStatus FROM `users`, `booking_details`, `service_stations`, `station_ports` WHERE `users`.`id` = `booking_details`.`customerKey` AND `booking_details`.`stationKey` = `service_stations`.`id` AND `booking_details`.`stationPortKey`=`station_ports`.`fldStationPortKey` AND `users`.`id` = ?";
    DB.query(sql, [customerId], async (err, result) => {
        if (err) {
            return res
                .status(200)
