...
const { createPaymentOder, verifyPaymentOrder } = require("./paymentHelper");
    ...
    const checkUser = DB.getConnection(async (err, connection) => {
        ...
        if (err) {
            ...
            var test = DB.query(sql, [phone, otp], async (err, result) => {
                ...
                let data = result[0];
                jwt.sign({ userId: data['id'], userRole: data['role'] }, jwtKey, { expiresIn: "2h" }, (err, token) => {
                    if (err) {
                        console.log(`err ccc`);
                    }
                    if (err) throw err;
...
exports.slotBookingByDate = async (req, res) => {
    ...
    const checkUser = DB.getConnection(async (err, connection) => {
        ...
        if (err) {
            ...
            let isBooking = `SELECT * FROM booking_details WHERE stationKey='${stationKey}' AND bookingDate='${bookingDate}'`;
...
exports.saveCustomerCarDetails = async (req, res) => {
    const customerKey = req.jwtUser['userId'];
...
exports.paymentCreateOrder = async (req, res) => {
    ...
    if(!bookingId) {
        return res
        .status(400)
        .json(returnError('Booking Id is missing.'));
    }
    const customerKey = req.jwtUser['userId'];
    return await createPaymentOder(bookingId, customerKey, res)
}

// create order
exports.verifyPaymentOrder = async (req, res) => {
    const {razorpay_payment_id, razorpay_order_id, razorpay_signature, paymentMode, paymentId} = req.body;
    return await verifyPaymentOrder(razorpay_payment_id, razorpay_order_id, razorpay_signature, paymentMode, paymentId)
}
